...
namespace usd
{
    ...
    bool _rigidBody)
  {
    ...
    math::Pose3d pose;
    auto poseErrors = PoseWrtParent(_link, pose);
    if (!poseErrors.empty())
    {
    ...
    for (uint64_t i = 0; i < _link.VisualCount(); ++i)
    {
      ...
      visualPath = validPath(visualPath);
    ...
    for (uint64_t i = 0; i < _link.CollisionCount(); ++i)
    {
      ...
      collisionPath = validPath(collisionPath);
    ...
    for (uint64_t i = 0; i < _link.SensorCount(); ++i)
    {
      ...
      sensorPath = validPath(sensorPath);
      UsdErrors errorsSensor = ParseSdfSensor(
