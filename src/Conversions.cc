...
namespace gz
{
  ...
  sdf::Material convert(const common::Material *_in)
  {
    ...
    const common::Pbr * pbr = _in->PbrMaterial();
    if (pbr != nullptr)
    {
      ...
      if (pbr->NormalMapType() == common::NormalMapSpace::TANGENT)
      {
      ...
      if (pbr->Type() == common::PbrType::SPECULAR)
      {
  ...
  void convert(const sdf::Material &_in, common::Material &_out)
  {
    ...
    if (pbr != nullptr)
    {
      ...
      if (pbrWorkflow)
      {
        pbrOut.SetType(common::PbrType::METAL);
      }
      else
      {
        ...
        if (pbrWorkflow)
        {
          pbrOut.SetType(common::PbrType::SPECULAR);
        }
      }
