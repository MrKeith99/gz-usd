...
TEST(Conversions, SdfToCommonMaterial)
{
  ...
  usd::convert(material, materialCommon);
  const common::Pbr * pbrCommon = materialCommon.PbrMaterial();
  ...
  EXPECT_EQ(common::NormalMapSpace::TANGENT,
    pbrCommon->NormalMapType());
  EXPECT_EQ(common::PbrType::METAL, pbrCommon->Type());
...
TEST(Conversions, CommonToSdfMaterial)
{
  ...
  common::Pbr pbrCommon;
  pbrCommon.SetType(common::PbrType::METAL);
  ...
  pbrCommon.SetNormalMap(
    "NormalMap", common::NormalMapSpace::TANGENT);
  ...
  const sdf::Material material = usd::convert(&materialCommon);
